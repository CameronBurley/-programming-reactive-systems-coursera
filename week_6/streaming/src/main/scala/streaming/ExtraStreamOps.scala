      ...
      val l = new GraphStage[FlowShape[T, T]] {
        ...
        override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape)
          ...
          override def onPush(): Unit = {
            ...
            println(s"push [${els.toString.take(9).padTo(9, ' ').replace('\n', ' ')}] >>> {$paddedName}")
            push(out, el)
          }

          override def onPull(): Unit = {
            println(s"pull             <~~ {$paddedName}")
            pull(in)
          }

          override def onUpstreamFinish(): Unit = {
            println(s"complete         ==> {$paddedName}")
          }
          override def onUpstreamFailure(ex: Throwable): Unit = {
            println(s"fail(onError)    xx> {$paddedName}")
          }
          override def onDownstreamFinish(cause: Throwable): Unit = {
            println(s"cancel           x~~ {$paddedName}")
          }
      ...
      val r = new GraphStage[FlowShape[T, T]] {
        ...
        override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape)
          ...
          override def onPush(): Unit = {
            ...
            println(s"push                 {$paddedName} >>> [${els.toString.take(9).padTo(9, ' ').replace('\n', ' ')}]")
            push(out, el)
          }

          override def onPull(): Unit = {
            println(s"pull from downstr    {$paddedName} <~~")
            pull(in)
          }
          ...
          override def onUpstreamFailure(ex: Throwable): Unit = {
            println(s"fail(onError)        {$paddedName} xx>")
          }
          override def onDownstreamFinish(cause: Throwable): Unit = {
            println(s"cancel from downstr  {$paddedName} x~~")
          }
